...
/**
 * Create assessor credentials (email + temporary password).
...
export async function POST(request) {
  ...
  await recordDirectorAudit({
    actor: gate.profile,
    action: "assessor.created",
    entityType: "profile",
    entityId: result.userId,
    newValue: { email: result.email, role: "assessor" },
    request,
  });
  ...
  return NextResponse.json({
    ...
    login_url: "/assessor-login",
    message:
      "Assessor account created. Copy the temporary password now — it will not be shown again. Share it securely. Assessor signs in at /assessor-login.",
  });
}
