...
export default function ApplicantChatPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: memberList } = await supabase.from("conversation_members").select("user_id").eq("conversation_id", conversationId);
      if (memberList) {
        const ids = memberList.map((m) => m.user_id);
        const { data: profiles } = await supabase.from("profiles").select("id, full_name, photo_url").in("id", ids);
        const map = {};
  ...
  async function sendMessage(e) {
    ...
    setSending(true);
    await supabase.from("messages").insert({ conversation_id: conversationId, sender_id: userId, content: newMsg.trim() });
    setNewMsg("");
    setSending(false);
  }
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col">
      <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
        <Link href="/applicant/messages" className="text-gray-400 hover:text-royal"><ArrowLeft size={18} /></Link>
      ...
      <form onSubmit={sendMessage} className="flex items-center gap-2 border-t border-gray-100 pt-4">
        <input type="text" value={newMsg} onChange={(e) => setNewMsg(e.target.value)} placeholder="Type a message..." className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-gold focus:ring-2 focus:ring-gold/20" />
        <button type="submit" disabled={!newMsg.trim() || sending} className="flex h-10 w-10 items-center justify-center rounded-xl bg-royal text-white hover:bg-royal-light disabled:opacity-50">
