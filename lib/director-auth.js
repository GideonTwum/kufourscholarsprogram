...
import { assertDirectorAal2 } from "@/lib/director-mfa";

/**
 * Active director: authenticated + profiles.role=director + is_active + MFA when required.
 * Prefer this for all privileged Director APIs.
 *
 * @param {{ requireMfa?: boolean }} [options]
 *   requireMfa defaults to true. Pass false only for non-privileged MFA bootstrap routes.
 *   When DIRECTOR_MFA_REQUIRED=false, AAL2 is skipped even if requireMfa is true.
 */
export async function requireActiveDirector(options = {}) {
  ...
  if (requireMfa) {
    // assertDirectorAal2 no-ops when DIRECTOR_MFA_REQUIRED=false
    const mfa = await assertDirectorAal2(supabase);
