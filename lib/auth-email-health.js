...
export function buildAuthEmailHealth(opts = {}) {
  ...
  return {
    ...
    auth_confirm_url: siteUrl
      ? `${siteUrl}/auth/confirm`
      : "(set NEXT_PUBLIC_SITE_URL)",
    password_reset_callback_url: siteUrl
      ? passwordResetCallbackUrl(siteUrl)
      : "(set NEXT_PUBLIC_SITE_URL)",
    mfa_required_for_director: false,
    applicant_verification: "email_confirmation_token_hash",
    typed_otp: false,
    sms_otp: false,
    manual_checks: {
      supabase_site_url: "Verify manually in Supabase Dashboard → Authentication → URL Configuration",
      supabase_redirect_allowlist:
        "Verify /auth/confirm, /auth/callback, and production site origin are allowlisted",
      supabase_confirm_signup_template:
        "Confirm signup must use TokenHash → /auth/confirm (docs/AUTH-CONFIRM-SIGNUP-TEMPLATE.md)",
      supabase_confirm_email: "Verify Confirm email is enabled for applicants",
      supabase_mfa:
        "Director MFA/TOTP is currently unused in app auth (password + role + is_active only)",
      resend_domain:
