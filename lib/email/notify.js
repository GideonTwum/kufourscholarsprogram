...
export async function sendTransactionalEmail({ to, subject, html, text }) {
  ...
  if (!cfg.canSend || !cfg.emailFrom) {
    const reason = cfg.fromError || (!cfg.resendApiKey ? "no_api_key" : "email_from_unavailable");
    if (process.env.NODE_ENV === "development") {
      console.info("[KSP Email] skipped (sender/config)", {
        event: "direct_resend",
        reason,
        to: recipients,
        subject,
...
${k.interviewInstructions ? `<p><strong>Instructions:</strong><br/>${escapeHtmlWithBreaks(k.interviewInstructions)}</p>` : ""}
<p>Best,<br/>The Kufuor Scholars Program Team</p>`;
}

export function stage1SubmittedEmailHtml(fullName, applicationClassName) {
  const name = fullName || "Applicant";
  const classClause = applicationClassName
    ? ` ${escapeHtml(String(applicationClassName).trim())}`
    : "";
  return `
    <p>Dear ${escapeHtml(name)},</p>
    <p>Thank you for submitting your Stage 1 application to the Kufuor Scholars Program${classClause}.</p>
