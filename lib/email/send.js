...
export async function sendKspEmail({
  ...
  if (!cfg.canSend) {
    const reason = cfg.fromError || (cfg.missing.length ? cfg.missing.join(",") : "configuration");
    if (cfg.isProduction || process.env.NODE_ENV === "production") {
      console.error(
        `[KSP Email] cannot send (${event || "unknown"}): ${reason}`
      );
    }
    await logEmailEvent({
      event,
