...
/** Public Apply / register entry — must never funnel staff into Director routes. */
 ...
 *
 * Critical: staff on Apply/register must NOT bounce to /director.
 */
export function authRouteBouncePath(pathname, role) {
  if (!role) return null;

  if (pathname.startsWith("/forgot-password")) return null;
  if (pathname.startsWith("/account-recovery")) return null;
 ...
 * Never send non-directors to /director/*.
 */
