...
} from "./applicant-demographics.js";
...
/**
 * Age eligibility (canonical): applicant must be 23 years old or younger
 * at the eligibility reference date (default: now / application time).
 * Rule: age <= MAX_APPLICANT_AGE (23 inclusive; 24+ ineligible).
 * Age is birthday-aware (not year subtraction alone).
 */
export const MAX_APPLICANT_AGE = 23;
export const AGE_ELIGIBILITY_MESSAGE =
  "Applicants must be 23 years old or younger to be eligible.";

/** Applicant-facing Stage 1 document labels (internal fields may differ). */
export const PASSPORT_PICTURE_LABEL = "Passport Picture";
/** Legacy DB column: student_id_path — UI meaning is National ID. */
export const NATIONAL_ID_LABEL = "National ID";
export const NATIONAL_ID_FIELD = "student_id_path";

/**
 * Birthday-aware age in whole years on `ref` (defaults to now).
 * Uses calendar DOB YYYY-MM-DD in local timezone (no UTC date shift).
 * @param {string} dateOfBirthStr
 * @param {Date} [ref]
 * @returns {number|null}
 */
export function ageInYearsOnDateOfBirth(dateOfBirthStr, ref = new Date()) {
  ...
  // Reject impossible calendar dates (e.g. 2002-02-31 → Mar 3)
  if (dob.getFullYear() !== y || dob.getMonth() !== m || dob.getDate() !== d) {
    return null;
  }
  const asOf = ref instanceof Date && !Number.isNaN(ref.getTime()) ? ref : new Date();
...
/** @returns {boolean} true when age is known and <= MAX_APPLICANT_AGE */
export function isAgeEligible(dateOfBirthStr, ref = new Date()) {
  const age = ageInYearsOnDateOfBirth(dateOfBirthStr, ref);
  return age !== null && age >= 0 && age <= MAX_APPLICANT_AGE;
}
...
export function validatePersonalInfo(data) {
  ...
  if (!data.date_of_birth) errors.date_of_birth = "Date of birth is required";
    ...
    if (age === null) errors.date_of_birth = "Please enter a valid date of birth";
      ...
      errors.date_of_birth = AGE_ELIGIBILITY_MESSAGE;
    }
  }
...
export const MIN_WASSCE_RESULTS = 1;
export const MAX_WASSCE_RESULTS = 5;

export const WASSCE_RESULTS_HINT =
  "Upload your WASSCE results/certificate and, where applicable, any NovDec results/certificates.";

export const WASSCE_HISTORICAL_MESSAGE =
  "Not collected when this application was submitted.";

export const ACADEMIC_TRANSCRIPT_HINT =
  "Upload your current university or tertiary academic transcript / academic record.";

/** WASSCE / NovDec result document paths from jsonb array. */
export function getWassceResultsPaths(data) {
  const arr = data?.wassce_results_urls;
  if (Array.isArray(arr) && arr.length) {
    return arr.filter((p) => typeof p === "string" && p.trim()).map((p) => p.trim());
  }
  return [];
}

export function hasWassceResults(data) {
  return getWassceResultsPaths(data).length > 0;
}

/**
 * Staff / read-only views: empty WASSCE list on a non-editable application
 * means the application was submitted before this requirement existed.
 */
export function shouldShowWassceHistoricalNote(data, { isEditable = false } = {}) {
  if (isEditable) return false;
  return !hasWassceResults(data);
}
...
export function validateDocuments(data) {
  ...
  if (wasscePaths.length < MIN_WASSCE_RESULTS) {
    errors.wassce_results_urls =
      "Please upload at least one WASSCE / NovDec results document before submitting Stage 1.";
  } else if (wasscePaths.length > MAX_WASSCE_RESULTS) {
    errors.wassce_results_urls = `You may upload at most ${MAX_WASSCE_RESULTS} WASSCE / NovDec documents.`;
  }
  ...
  } else if (recommendations.length > MAX_RECOMMENDATION_LETTERS) {
    errors.recommendation_urls = `You may upload at most ${MAX_RECOMMENDATION_LETTERS} recommendation letters.`;
  }
  if (!data.photo_url?.trim()) {
    errors.photo_url = `${PASSPORT_PICTURE_LABEL} is required (JPG, PNG, or WebP, max 5MB)`;
  }
  if (!hasStoragePath(data.student_id_path)) {
    errors.student_id_path = `${NATIONAL_ID_LABEL} document is required`;
  }
